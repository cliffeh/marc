...
static void marcfield_print_subfields(FILE *out, const marcfield *field,
  ...
  while (i < field->length) {
    ...
    case SUBFIELD_DELIMITER: // state change; re-assess whether we should be
                             // printing
    {
      i++;
...
int marcrec_print(FILE *out, const marcrec *rec, const fieldspec specs[]) {
  if (!specs) // TODO update spec to include leader fields?
  {
    ...
    fprintf(out,
            "character coding scheme: %c | indicator count: %c | subfield code "
            "count: %c\n",
            rec->data[10], rec->data[11], rec->data[12]);
    fprintf(out,
            "base address of data: %.5s | encoding level: %c | descriptive "
            "cataloging form: %c\n",
            rec->data + 12, rec->data[17], rec->data[18]);
    fprintf(out,
            "multipart resource record level: %c | length of the "
            "length-of-field portion: %c\n",
